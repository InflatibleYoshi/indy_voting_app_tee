...
fn main() {
    ...
    let wallet_handle: WalletHandle = wallet::open_wallet(&config, USEFUL_CREDENTIALS).wait().unwrap();
    ...
    let prover_wallet_handle: WalletHandle = wallet::open_wallet(&prover_wallet_config, USEFUL_CREDENTIALS).wait().unwrap();
